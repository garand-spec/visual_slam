...
浏览器查看摄像头的三维位置、朝向和分段轨迹：

```bash
./run_pose_viewer.sh --port 8765
```

同一局域网打开 `http://192.168.4.124:8765/`。页面支持历史回放、实时位置、跟随视角和参考点云。实时查看需另外启动 `./run_visual_slam_3d.sh --no-viewer`；网页不会自行打开摄像头。

摄像头固定佩戴时可以作为人的运动参考，但不等于人体姿态或真实房间坐标。本次现场检查频繁初始化并丢失，尚不能稳定测量行走轨迹；零地图支持的初始化位置会被显示层判为无效。功能边界、实测问题与操作步骤见 [三维轨迹可视化说明](docs/三维轨迹可视化.md)。

## 场景与物体表面建模

新版可在采集结束后按最终位姿生成灰度三角网格，在网页查看并下载 PLY/OBJ。初始化和重定位也已加强；当前短时实测因有效双目深度不足，尚未生成真实场景模型。使用方法、限制与验证见 [场景表面建模说明](docs/场景表面建模与连续性.md)。
...
实时模式现在可同时查看摄像头位置和持续更新的稠密/稀疏点云，无需先结束采集，说明与实测证据见 [实时点云](docs/实时点云_20260913.md)。


2026-09-14：已移除深度归档 2,000 组截断，每组深度保留独立采集位姿和地图版本。最终位姿缺失时可按独立版本恢复未优化模型，说明与验证见 [长时采集数据保全修复](docs/长时采集数据保全修复_20260914.md)。
