...
2026-09-12 更新：项目架构、已有运行统计、验证范围和待完善项见 [项目理解与进展报告 v2.0](docs/视觉SLAM方案.md)。本文保留操作说明；性能数字均需结合各次运行条件解释。
...
此模式显示 `ORB-SLAM3: 3D Sparse Map`，左侧面板给出跟踪状态；鼠标左键旋转，滚轮缩放。历史文档记录过 X11/Pangolin 与地图线程争用，特定场景下降至约 2 FPS，以及 OpenCV 窗口约 21 FPS；相关完整参数与日志未留存，本次未复测。这些数字不作为当前配置的性能承诺，Pangolin 模式主要用于检查稀疏地图结构。
...
- `poses.csv`：每帧状态、地图点数量和世界到相机变换 `Tcw`，时间单位为秒；绘制世界坐标相机中心前需取逆。
- `tracking_health.csv`：状态名、特征数、处理时延、连续丢失帧数等诊断数据。
- `status.txt`：当前跟踪健康状态，供 `status_slam.sh` 读取。
- `CameraTrajectory.txt`、`KeyFrameTrajectory.txt`：ORB-SLAM3 轨迹，使用纳秒时间戳；最终轨迹与在线 `poses.csv` 的方向、参考地图和优化状态可能不同。

稠密点云按提交时位姿累计，目前没有回环后重新融合或地图切换隔离；跟踪状态正常也不等于稠密地图全局一致。
...
左右目 CUDA ORB 使用各自的非默认 CUDA Stream，并复用 `GpuMat`、主机描述子矩阵和关键点容器，避免左右目在 `Stream::Null()` 上串行以及逐帧重复分配 GPU 内存。历史说明记录过 120W 模式与 `jetson_clocks`，本次未核实当前功耗或锁频状态；性能对比时应另行记录这些条件。

本次读取的最新保留记录 `run_20260912_192042` 有 660 帧，成功跟踪比例 100%，相机时间戳跨度 30.027 秒，处理采样率约 21.95 帧/s；`TrackStereo` 平均耗时 43.964 ms、P95 为 63.506 ms，最终导出 432 个稀疏点和 367276 个稠密点。该记录没有保存完整启动参数或源码提交，且本次没有重新采集，不能据此承诺长期稳定性或定位精度。

早期记录 `run_20260831_151502` 的处理采样率约 29.08 帧/s、平均跟踪耗时 19.726 ms，但成功跟踪比例为 71.96%。不同场景和设置不能直接横向比较。`track_ms` 只覆盖 SLAM 跟踪调用，不能取倒数替代完整系统帧率。详细计算规则和原始文件哈希见 [运行核查数据](docs/运行核查_20260912.json)。
